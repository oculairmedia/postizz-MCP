 ...
 */
export async function handleCreatePost(server, args) {
    try {
        // Destructure and validate required arguments
        const { content, integration_id, post_type = "now", publish_date = "2025-02-27T19:30:00.000Z", media_urls = [], short_link = false } = args;
        // Validate content
        if (!content || typeof content !== 'string' || content.length < 6) {
            throw new McpError(ErrorCode.InvalidParams, "Content must be at least 6 characters long");
        }
        // Validate post_type
        if (!["draft", "schedule", "now"].includes(post_type)) {
            throw new McpError(ErrorCode.InvalidParams, 'post_type must be one of: "draft", "schedule", "now"');
        }
...
export const createPostToolDefinition = {
    ...
    inputSchema: {
        ...
        required: ['content', 'integration_id']
    }
};
