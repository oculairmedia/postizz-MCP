...
// Configuration
const BOOKSTACK_BASE_URL = 'https://bookstack.oculair.ca/v1';
const BOOKSTACK_PASSWORD = 'bookstackSecurePass123';
...
async function testAgentCommunication() {
    try {
        ...
        const headers = {
            ...
            'X-BARE-PASSWORD': `password ${BOOKSTACK_PASSWORD}`,
'Authorization': `Bearer ${BOOKSTACK_PASSWORD}`,
            'user_id': AGENT_ID
        };
        // First, verify agent exists
        console.log('Checking agent...');
        const agentInfo = await axios.get(`${BOOKSTACK_BASE_URL}/agents/${AGENT_ID}`, { headers });
        ...
        const response = await axios.post(`${BOOKSTACK_BASE_URL}/agents/${AGENT_ID}/functions`, {
